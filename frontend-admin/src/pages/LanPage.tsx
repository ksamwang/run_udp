        ...
        <Card>
          <Table
            ...
            columns={[
              ...
              { title: 'Relay', dataIndex: 'relay_disabled', render: (v) => <Tag color={v ? 'red' : 'green'}>{v ? '禁用' : '可用'}</Tag> },
              { title: '会话/热路径', render: (_, row) => `${row.active_sessions || 0} / ${row.hot_paths || 0}` },
              { title: 'Socket Rotation', render: (_, row) => row.socket_rotations ? `${row.socket_rotations} ${row.last_rotation_reason || ''}` : '-' },
