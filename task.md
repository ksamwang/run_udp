...
- [ ] 封装 packet frame：
  ...
  - `packet_type`
  - `payload`
- [ ] 处理 ICMP/TCP/UDP 基础转发。
- [ ] 统计 tx/rx bytes、drop reason。

验收：

- A 到 B 的 ICMP echo request/response 可以转发。
- 被 ACL 拒绝的包可计数和上报。
...
- 虚拟网段默认值是否使用 `172.16.10.0/24`？
- LAN 和当前端口转发是否共用同一个设备 ID？
- 设备密钥如何初始化和恢复？
- 是否需要按用户/租户隔离虚拟网络？
- 是否需要支持多个虚拟网络？
- 是否要优先支持 ICMP，还是直接以 TCP/RDP 验收？
