...
const windowsRunKeyName = "UDPTunnelLANTray"
...
func runWindowsService(run func(context.Context)) error {
	return svc.Run(lan.ServiceName, &serviceRunner{run: run})
}
...
func installWindowsService(exePath, configPath string) error {
	...
	defer m.Disconnect()
	if s, err := m.OpenService(lan.ServiceName); err == nil {
		defer s.Close()
		cfg, err := s.Config()
		if err != nil {
			return err
		}
		cfg.DisplayName = lan.ProductName
		cfg.StartType = mgr.StartAutomatic
		cfg.BinaryPathName = serviceBinaryPath(exePath, configPath)
		if err := s.UpdateConfig(cfg); err != nil {
			return err
		}
		return setServiceRecovery(lan.ServiceName)
	}
	s, err := m.CreateService(lan.ServiceName, exePath, mgr.Config{
		DisplayName: lan.ProductName,
...
func uninstallWindowsService() error {
	...
	defer m.Disconnect()
	s, err := m.OpenService(lan.ServiceName)
	if err != nil {
		return nil
	}
	defer s.Close()
...
func setServiceRecovery(name string) error {
	return exec.Command("sc.exe", "failure", name, "reset=", "86400", "actions=", "restart/5000/restart/5000/restart/10000").Run()
}
