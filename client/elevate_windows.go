...
func saveClientConfigWithElevation(configPath string, cfg config.Client) (bool, error) {
	...
	tmp, err := os.CreateTemp("", "udp-tunnel-client-*.json")
	if err != nil {
		return false, fmt.Errorf("create elevated config temp: %w", err)
	}
	tmpPath := tmp.Name()
	...
	if err := config.SaveClientLocalJSON(tmpPath, cfg); err != nil {
		_ = os.Remove(tmpPath)
		return false, err
	}
	if err := runElevatedSelf("-apply-client-config", tmpPath, "-config", configPath); err != nil {
		_ = os.Remove(tmpPath)
		return false, err
	}
	return true, nil
