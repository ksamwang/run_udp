...
- [ ] 增大 poll 批量上限，降低高包率下的请求次数。
- [ ] 降低服务端空队列唤醒延迟，减少 `100ms` ticker 对交互延迟的影响。
- [ ] 复用 HTTP client，避免每次 `postLANJSON` 新建 client。
- [ ] 评估 gzip 是否对小包有负收益，避免盲目启用压缩。
- [ ] 增加 relay queue 溢出、延迟、批量大小的指标和日志。
...
- [ ] 减少不必要的 payload copy，尤其是路由和 relay 封装阶段。
...
- [ ] 客户端日志明确输出每个 peer 当前路径：`p2p_datagram`、`p2p_kcp`、`relay_udp`、`relay_http`。
- [ ] 管理后台展示每个 LAN peer 的当前路径、RTT、估算吞吐、最近切换原因。
- [ ] 记录路径切换时间线，便于判断是否频繁抖动。
- [ ] 增加一键导出 UDPTunnelLAN 诊断信息。
